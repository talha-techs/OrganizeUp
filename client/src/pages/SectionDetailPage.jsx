...
const SectionDetailPage = () => {
  ...
  const { currentSection, isLoading } = useSelector((state) => state.sections);
  ...
  const hasFolders = currentSection.folders && currentSection.folders.length > 0;
  const hasRootFiles =
    currentSection.files &&
    currentSection.files.filter((f) => !f.path || !f.path.includes('/')).length > 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              {currentSection.files?.length || 0} files
              {currentSection.addedBy?.name && ` · by ${currentSection.addedBy.name}`}
            </p>
          </div>
      ...
      {/* Content */}
      {currentSection.files?.length === 0 && !hasFolders ? (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {/* Folder tree (if imported with hierarchy) */}
          {hasFolders && (
            <div className="mb-4">
              <h4 className="text-sm font-medium text-slate-300 px-3 mb-2">Folders</h4>
              {currentSection.folders.map((folder, i) => (
                <FolderTree
