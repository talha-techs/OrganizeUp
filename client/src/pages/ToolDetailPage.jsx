...
const ToolDetailPage = () => {
  ...
  const isOwner =
    !!(user?._id && currentTool?.addedBy &&
      String(currentTool.addedBy?._id ?? currentTool.addedBy) === String(user._id));
  ...
  const handleRemoveFile = async (fileId) => {
    ...
    const result = await dispatch(removeFileFromTool({ toolId: id, fileId }));
    if (result.meta.requestStatus === 'fulfilled') {
  ...
  const hasFolders = currentTool.folders && currentTool.folders.length > 0;
  const hasFiles = currentTool.files && currentTool.files.length > 0;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            {(currentTool.link || currentTool.driveLink) && (
              <a
                href={currentTool.driveLink || currentTool.link}
                target="_blank"
      ...
      <DriveImportModal
        ...
        title={`Import to "${currentTool.title}"`}
      />
    </div>
  );
};
